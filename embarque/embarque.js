...
const postUser = async(data)=>{
    ...
    let respuesta = await ( await fetch("http://localhost:3000/embarque",config)).json();
...
const getUserAll = async()=>{
    ...
    respuesta.forEach(pasajero => {
        tabla.innerHTML +=`
        <tr class="registro" id='${pasajero.id}'>
                <td>${pasajero.documento}</td>
                <td>${pasajero.fNacimiento}</td>
                <td>${pasajero.nVuelo}</td>
...
const deleteUser = async(id)=>{
    ...
    let respuesta = await ( await fetch(`http://localhost:3000/embarque/${id}`,config)).json();
    console.log(respuesta);
}
...
const searchUser = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:3000/embarque?q=${Object.values(data).join("")}`,config)).json();
    console.log(res);
}   
