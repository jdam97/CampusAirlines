...
const postUser = async(data)=>{
    ...
    let respuesta = await ( await fetch("http://localhost:3000/flotas",config)).json();
...
const getUserAll = async()=>{
    ...
    respuesta.forEach(flota => {
        tabla.innerHTML +=`
        <tr class="registro" id='${flota.id}'>
                <td>${flota.numero}</td>
                <td>${flota.nPasajeros}</td>
                <td>${flota.fechaCompra}</td>
                <td>${flota.valorCompra}</td>
                <td>${flota.nMatricula}</td>
...
const deleteUser = async(id)=>{
    ...
    let respuesta = await ( await fetch(`http://localhost:3000/flotas/${id}`,config)).json();
    console.log(respuesta);
}
