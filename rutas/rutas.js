...
const postUser = async(data)=>{
    ...
    let respuesta = await ( await fetch("http://localhost:3000/rutas",config)).json();
...
const getUserAll = async()=>{
    ...
    respuesta.forEach(ruta => {
        tabla.innerHTML +=`
        <tr class="registro" id='${ruta.id}'>
...
const deleteUser = async(id)=>{
    ...
    let respuesta = await ( await fetch(`http://localhost:3000/rutas/${id}`,config)).json();
    console.log(respuesta);
}
